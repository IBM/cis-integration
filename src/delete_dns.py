    ...
    def delete_dns(self):
        ...
        if len(root_id) > 0:
            try:
                ...
                print(Color.GREEN+"SUCCESS: Deleted '@' record")
            except ApiException as ae:
                print(Color.RED + "ERROR: Could not delete '@' record with message: " + ae.http_response + " " + ae.message + Color.END) 
        if len(www_id) > 0:
            try:
                www_delete = record.delete_dns_record(
                    dnsrecord_identifier=www_id
                ).get_result()
                print(Color.GREEN+"SUCCESS: Deleted 'www' record")
            except ApiException as ae:
                print(Color.RED + "ERROR: Could not delete 'www' record with message: " + ae.http_response + " " + ae.message + Color.END) 